...
| `langgraph_checkpoint` | `JSONB` | nullable | Raw LangGraph checkpoint (serialized state for crash recovery). Distinct from `state_snapshot` — see note below. |
...
| `state_snapshot` | `JSONB` | nullable | Human-readable JSONB state snapshot of the full LangGraph `ApplicantState` (added in migration `20260726001`). Distinct from `langgraph_checkpoint`: `state_snapshot` stores the deserialized, application-level state for inspection and recovery, while `langgraph_checkpoint` stores the raw LangGraph-internal checkpoint binary used by `PostgresSaver` for exact session resume.
...
The following columns contain personally identifiable information and are automatically redacted in application logs by the structlog PII redaction processor (see [Security & Privacy Document](security-privacy.md) Section 2):

- `applicants.identity_number`, `applicants.full_name`, `applicants.phone`, `applicants.email`
- `emirates_id_data.identity_number`, `emirates_id_data.full_name_en`, `emirates_id_data.full_name_ar`
- `bank_statement_data.account_number`, `bank_statement_data.account_holder_name`
- `credit_report_data.identity_number`, `credit_report_data.full_name`, `credit_report_data.contact_details`
- `resume_data.full_name`, `resume_data.email`, `resume_data.phone`
- `application_form_data.applicant_name`, `application_form_data.identity_number`, `application_form_data.contact_phone`, `application_form_data.contact_email`
