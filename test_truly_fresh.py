...
def test_auth(emirates_id: str) -> dict:
    ...
    logger.info(
        ...
        is_new=data.get("is_new_applicant"),
    )
    return data
...
def test_chat(application_id: str, message: str, files: list[str] | None = None) -> dict:
    ...
    if resp.status_code == 200:
        ...
        logger.info(
            ...
            document_count=len(data.get('uploaded_documents', [])),
        )
        return data
    else:
...
def main():
    ...
    logger.info("test_start", script="test_truly_fresh")

    # Generate new Emirates ID
    emirates_id = generate_new_emirates_id()
    logger.info("emirates_id_generated", script="test_truly_fresh")
    ...
    logger.info("test_complete", script="test_truly_fresh", application_id=app_id, duration_ms=round(duration_ms, 2))
